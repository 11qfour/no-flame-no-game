...
class AdvancedPersonTracker:
    ...
    def update(self, detections: List[Tuple[int, int, int, int]], frame_number: int) -> Dict:
        ...
        # Прогнозируем следующую позицию на основе последних двух точек траектории
        predicted_positions = {}
    ...
    def _match_detections_to_trackers(self, detections: List[Tuple[int, int, int, int]], 
        ...
        # Жадное сопоставление с адаптивным порогом расстояния и проверкой направления движения
        max_distance = 140  # базовое значение
        ...
        for i in range(len(detections)):
            ...
            for j, person_id in enumerate(person_ids):
                # Адаптивный порог с учетом последнего шага траектории
                ...
                if person_id not in matched_trackers and distance_matrix[i, j] < adaptive_distance:
                    ...
                    if len(traj) >= 2:
                        ...
                        if nv > 0 and nd > 0:
                            cosang = dot / (nv * nd)
                            # если движение резко в обратную сторону, игнорируем (но допускаем, если шаги маленькие)
    ...
    def get_trajectories(self) -> Dict:
        ...
        for person_id, tracker in self.trackers.items():
            if len(tracker.trajectory) >= self.min_trajectory_length:
                traj = tracker.get_trajectory()
                # Отсекаем почти статичные траектории (дрожание детектора)
                if len(traj) >= 2:
                    total_dist = 0.0
